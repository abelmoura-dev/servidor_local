...
const verifyQueue = async (
  ...
  const botText = async () => {
    const currentAmountUsedBotQueues = ticket.amountUsedBotQueues;
    const novoValor = currentAmountUsedBotQueues + 1;

    // Atualize o ticket com o novo valor de amountUsedBotQueues
    await UpdateTicketService({
      ticketData: { amountUsedBotQueues: novoValor },
      ticketId: ticket.id,
      companyId: ticket.companyId
    });

    //verifica se bot executou a quantidade de vezes definida (anti flood bot)
    if(ticket.amountUsedBotQueues >= maxUseBotQueues){
      console.log("Antiflood ativado");
    }else{
  ...
  if (choosenQueue) {
    ...
    await UpdateTicketService({
      ticketData: { queueId: choosenQueue.id, chatbot, amountUsedBotQueues: 0 },
      ticketId: ticket.id,
      companyId: ticket.companyId,
    });
...
const handleChartbot = async (ticket: Ticket, msg: WAMessage, wbot: Session, dontReadTheFirstQuestion: boolean = false) => {
  ...
  const { maxUseBotQueues } = await ShowWhatsAppService(
    wbot.id!,
    ticket.companyId
  );
  ...
  if (!isNil(queue) && isNil(ticket.queueOptionId)) {
    ...
    const botText = async () => {
      ...
      // Atualize o ticket com o novo valor de amountUsedBotQueues
      await UpdateTicketService({
        ticketData: { amountUsedBotQueues: novoValor, chatbot: true },
        ticketId: ticket.id,
        companyId: ticket.companyId
      });
    
      // Verifica se bot executou a quantidade de vezes definida (antiflood bot)
      if (ticket.amountUsedBotQueues >= maxUseBotQueues) {
        console.log("Antiflood ativado");
        await UpdateTicketService({
