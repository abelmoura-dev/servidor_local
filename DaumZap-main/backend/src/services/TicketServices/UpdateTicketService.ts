...
const UpdateTicketService = async ({
  ...
  try {
    ...
    const { amountUsedBotQueues } = ticketData;

    // Verificar se amountUsedBotQueues foi fornecido
    if (typeof amountUsedBotQueues !== 'undefined') {
      // Atualizar ticketData com o novo valor de amountUsedBotQueues
      ticketData = { ...ticketData, amountUsedBotQueues };
    }
    ...
    if (status !== undefined && ["closed"].indexOf(status) > -1) {
      ...
      // Definir amountUsedBotQueues como 0
      ticketData.amountUsedBotQueues = 0;

      ticket.amountUsedBotQueues = ticketData.amountUsedBotQueues;
      await ticket.update({
